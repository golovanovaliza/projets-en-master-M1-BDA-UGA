...
dfadjustSE <- function(model, clustervar=NULL, ell=NULL, IK=TRUE, tol=1e-9,
  ...
  if(is.null(clustervar)) {
    ...
    AQf <- function(S) {
      ...
      Ds <- e$vectors %*% ((1-e$values >= 1e-9) *
                             (1/sqrt(pmax(1-e$values, 1e-9))) * t(e$vectors))
      Qs %*% Ds
    }
...
write.csv(data.frame(y = y, x=x ),'test_df.csv')


ell = c(1, 0, rep(0,fm$rank-2))

dfadjustSE(fm)
# Clustering, with 5 clusters
clustervar <- as.factor(c(rep(1, 40), rep(1, 20),
                         rep(2, 20), rep(3, 10), rep(4, 10)))
dfadjustSE(fm, IK = FALSE, ell=ell, rho = TRUE)
